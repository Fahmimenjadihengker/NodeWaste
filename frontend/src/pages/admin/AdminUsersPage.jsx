    ...
    <div className="mx-auto max-w-7xl px-5 py-6 sm:px-8 lg:px-10 lg:py-8">
      <section className="rounded-[2rem] border border-leaf-900/10 bg-[#fffdf4] p-6 shadow-[0_18px_55px_rgba(32,58,37,0.08)] sm:p-8">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="grid grid-cols-3 gap-2 rounded-[1.25rem] bg-[#edf5e4] p-2 text-center">
            {['USER', 'DRIVER', 'ADMIN'].map((role) => <button key={role} className={`rounded-2xl px-4 py-3 text-xs font-black ${filter === role ? 'bg-leaf-800 text-cream' : 'text-leaf-900'}`} type="button" onClick={() => setFilter(role)}>{role}</button>)}
          </div>
        </div>
        ...
        <div className="mt-8 rounded-[1.5rem] border border-leaf-900/10 bg-[#f5f1df] p-5">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <button className="rounded-full bg-leaf-800 px-6 py-3 text-sm font-black text-cream shadow-lg shadow-leaf-950/15 transition hover:bg-moss" type="button" onClick={submit}>{form.id ? 'Simpan perubahan' : 'Buat akun'}</button>
          </div>
        ...
        <div className="mt-8">
          <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              <input className="rounded-full border border-leaf-900/10 bg-[#edf5e4] px-4 py-2 text-sm font-bold text-moss outline-none focus:border-leaf-700" placeholder="Cari akun..." value={query} onChange={(event) => setQuery(event.target.value)} />
              <select className="rounded-full border border-leaf-900/10 bg-[#edf5e4] px-4 py-2 text-sm font-black text-moss" value={filter} onChange={(event) => setFilter(event.target.value)}>

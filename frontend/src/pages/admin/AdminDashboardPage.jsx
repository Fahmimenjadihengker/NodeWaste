...
function StatCard({ label, value, detail, isLoading }) {
  return (
    <article className="rounded-[1.5rem] border border-leaf-900/10 bg-[#f5f1df] p-5 shadow-[0_18px_45px_rgba(32,58,37,0.08)]">
      <p className="text-sm font-black uppercase tracking-[0.18em] text-moss/45">{label}</p>
      {isLoading ? <Skeleton className="mt-5 h-10 w-16" /> : <p className="mt-5 text-4xl font-black tracking-[-0.05em] text-leaf-950">{value}</p>}
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-6 sm:px-8 lg:px-10 lg:py-8">
          ...
          <div className="rounded-[1.5rem] border border-leaf-900/10 bg-[#fbf7e8] p-5 shadow-[0_18px_45px_rgba(32,58,37,0.08)]">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <div className="rounded-2xl bg-[#f5f1df] p-4 text-leaf-950">
                {isLoading ? <Skeleton className="h-8 w-12" /> : <p className="text-3xl font-black">{stats.activeAccounts}</p>}
                <p className="text-xs font-bold text-moss/55">aktif</p>
              </div>
              <div className="rounded-2xl bg-[#f5f1df] p-4 text-leaf-950">
                {isLoading ? <Skeleton className="h-8 w-12" /> : <p className="text-3xl font-black">{stats.disabledAccounts}</p>}
      ...
      <section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {statCards.map((card) => <StatCard key={card.key} {...card} value={stats[card.key]} isLoading={isLoading} />)}
      </section>
    </div>
  )
}

...
function DriverProfileEditPage() {
  ...
  useEffect(() => {
    ...
    getDriverProfile().then((response) => {
      ...
      setDistrict({
        address: '-',
        districtName: data.driverProfile?.district?.name || '',
        city: data.driverProfile?.district?.city || '',
        province: data.driverProfile?.district?.province || '',
        provinceCode: data.driverProfile?.district?.provinceCode || '',
        cityCode: data.driverProfile?.district?.cityCode || '',
        districtCode: data.driverProfile?.district?.districtCode || '',
        latitude: '',
        longitude: '',
      })
  ...
  const handleSave = async () => {
    ...
    const hasDistrict = district.districtName && district.city && district.districtCode
    let closeLoading = null

    try {
      setIsSaving(true)
      closeLoading = sweetLoading({ title: 'Menyimpan profile...', text: 'Data driver sedang diproses.' })
  ...
  return (
    <div className="mx-auto max-w-5xl px-5 py-8 sm:px-8 lg:px-10 lg:py-12">
      <AppCard className="rounded-[1.75rem] shadow-[0_22px_70px_rgba(32,58,37,0.1)] sm:p-8">
        ...
        {isLoading ? <div className="mt-8 space-y-6"><div className="flex flex-col gap-5 sm:flex-row sm:items-center"><SkeletonText className="h-28 w-28 rounded-full" /><div className="flex-1 space-y-3"><SkeletonText className="h-4 w-32" /><SkeletonText className="h-12 w-full" /></div></div><div className="grid gap-4 sm:grid-cols-2"><SkeletonText className="h-16 w-full" /><SkeletonText className="h-16 w-full" /><SkeletonText className="h-16 w-full" /><SkeletonText className="h-16 w-full" /></div><SkeletonText className="h-64 w-full rounded-[1.25rem]" /></div> : <>
          ...
          <div className="mt-8 rounded-[1.25rem] bg-[#edf5e4] p-5">
            <AddressForm value={district} onChange={setDistrict} title="Wilayah kerja" heading="Wilayah kerja driver" description="Pilih wilayah kerja agar data driver tampil benar di profile dan map." />
          </div>
        </>}

        <div className="mt-8 flex flex-wrap gap-3">
          <button className="rounded-full bg-leaf-600 px-6 py-3 text-sm font-black text-white transition hover:bg-leaf-900 disabled:cursor-not-allowed disabled:opacity-60" type="button" onClick={handleSave} disabled={isLoading || isSaving}>{isSaving ? 'Menyimpan...' : 'Simpan profile'}</button>

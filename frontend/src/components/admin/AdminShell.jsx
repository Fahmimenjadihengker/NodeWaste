      ...
      <aside className="fixed bottom-0 left-0 top-0 z-40 hidden w-72 border-r border-leaf-900/10 bg-[#edf5e4] px-5 py-6 shadow-[18px_0_50px_rgba(32,58,37,0.10)] md:flex md:flex-col">
        ...
        <nav className="mt-8 grid gap-2" aria-label="Navigasi admin">
          {adminNavItems.map((item) => (
            <NavLink
              key={item.to}
              className={({ isActive }) => `flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-black transition ${isActive ? 'bg-[#fbf7e8] text-leaf-800 shadow-[0_14px_28px_rgba(32,58,37,0.10)] ring-1 ring-leaf-900/10' : 'text-moss/70 hover:bg-leaf-100 hover:text-leaf-950'}`}
              to={item.to}
            >
        ...
        <div className="mt-auto rounded-[1.5rem] border border-leaf-900/10 bg-[#fbf7e8] p-4 shadow-[0_18px_40px_rgba(32,58,37,0.08)]">
          ...
          <button className="mt-4 w-full rounded-2xl bg-red-700 px-4 py-3 text-sm font-black text-white shadow-lg shadow-red-950/15 transition hover:bg-red-800" type="button" onClick={handleLogout}>Keluar</button>
        </div>
      </aside>
